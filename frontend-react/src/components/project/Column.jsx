...
export default function Column({ status, tasks = [] }) {
    ...
    switch (status) {
        case 'AWAITING_COMPLETION':
            return column({ backgroundColor: 'var(--color-todo)' }, 'Нужно сделать', '0.25rem');
        case 'IN_PROGRESS':
            return column({ backgroundColor: 'var(--color-progress)' }, 'В работе', '0.75rem');
        case 'COMPLETED':
            return column({ backgroundColor: 'var(--color-done)' }, 'Готово', '0.75rem');
    }
}
